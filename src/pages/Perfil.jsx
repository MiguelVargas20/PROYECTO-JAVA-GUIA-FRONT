 ...
 *
 * Flujo:
 *  1. Toma el username del AuthContext (guardado al hacer login)
 *  2. Llama GET /api/usuarios/correo/{username} para obtener el UsuarioDto completo
 *  3. Muestra los datos y permite editarlos
 *  4. Guarda con PUT /api/usuarios/{id}
 *
 * UsuarioDto: { id, nombre, apellido, docId:{tipo,numero},
 *               telefono, correo, direccion:{ciudad,direccion},
 *               fechaNacimiento, estado, fechaRegistro }
 */
export default function Perfil() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      try {
        const { data } = await usuariosApi.obtenerPorCorreo(user.username);
        setPerfil(data);
        setForm({
          nombre:          data.nombre           || '',
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      ...
      await usuariosApi.actualizar(perfil.id, body);
    ...
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', paddingBottom: '3rem' }}>
      ...
      <Container style={{ maxWidth: '780px', position: 'relative', zIndex: 1 }}>
        ...
        <Form onSubmit={handleGuardar}>
          ...
          >
            <Row className="g-3">
              ...
              <Col md={6}>
                ...
                {/* Solo lectura — no se edita desde el perfil */}
                <Form.Control
                  value={docTexto}
                  readOnly
                  className="shadow-none campo-perfil"
          ...
          <SeccionCard icon="bi-telephone" titulo="Información de Contacto">
            <Row className="g-3">
              ...
              <Col md={12}>
                ...
                <Form.Control
                  name="direccion"
                  value={form.ciudad && form.direccion
          ...
          {/* ══ Sección 3: Credenciales de Cuenta ═════════ */}
